...
from utils import DEFAULTS, _coerce, get_setting, set_setting
...
def test_get_setting_corrupt_row_returns_default():
    ctx, mock_model = _with_mock_model()
    with ctx:
        mock_model.query.filter_by.return_value.first.return_value = FakeSettingsRow(
            "max_containers_per_user", "not-an-integer"
        )
        mock_app = MagicMock()
        mock_app.__bool__ = lambda self: True

        with patch.object(_flask_mod, "current_app", mock_app):
            assert get_setting("max_containers_per_user") == DEFAULTS["max_containers_per_user"]
