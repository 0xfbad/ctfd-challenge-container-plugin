...
def test_load_contexts_retains_failed_context_for_recovery():
    hm = DockerHostManager()
    ...
    assert "bad" in hm._context_configs
...
def test_call_does_not_deadlock_when_fn_invokes_clear_client():
    ...
    assert not any(k[0] == "default" for k in hm._clients)
    # Clearing the shared client and closing the per-call recovery client are
    # both expected; most importantly the nested call must not deadlock.
