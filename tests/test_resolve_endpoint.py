...
from docker_host_manager import LOCAL_CONTEXT_NAME, _resolve_endpoint, _scan_context_meta
...
def test_only_local_context_uses_socket_fallback(tmp_path):
    sock = tmp_path / "docker.sock"
    sock.touch()

    with patch("docker_host_manager.LOCAL_SOCKET_PATH", str(sock)):
        result = _resolve_endpoint(LOCAL_CONTEXT_NAME, None)
    assert result == f"unix://{sock}"

    with patch("docker_host_manager.LOCAL_SOCKET_PATH", str(sock)):
        assert _resolve_endpoint("missing-remote", None) is None
